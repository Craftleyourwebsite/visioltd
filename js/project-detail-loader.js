...
async function initProjectDetail() {
    ...
    try {
        // Optimize fetch: only request necessary fields and specific populations
        const fields = [
...
function injectProjectData(project) {
    ...
    const setText = (id, content) => {
        ...
        else el.textContent = ''; // Clear if null/undefined
    };
    ...
    const setImage = (id, content, isBackground = false) => {
        ...
        // If it's a wrapper with loading-skeleton, remove it from the wrapper or the element itself
        if (el.classList.contains('loading-skeleton')) {
            el.classList.remove('loading-skeleton');
        } else if (el.parentElement && el.parentElement.classList.contains('loading-skeleton')) {
            el.parentElement.classList.remove('loading-skeleton');
        }

        if (!content?.url) {
            if (el.tagName === 'IMG') el.style.opacity = '0.5';
            return;
        }
    ...
    try {
        ...
        // Double check: ensure ALL loading skeletons are removed even if injection fails for some elements
