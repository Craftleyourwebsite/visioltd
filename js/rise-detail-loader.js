/**
 * RISE DETAIL LOADER
 * Fetches specific rise item by slug and populates riseopen.html
 */

document.addEventListener('DOMContentLoaded', initRiseDetail);

async function initRiseDetail() {
    const urlParams = new URLSearchParams(window.location.search);
    const slug = urlParams.get('rise');

    if (!slug) {
        console.warn('No rise slug found in URL');
    ...
    try {
        const response = await fetch(`${CONFIG.API_URL}/rises?filters[slug][$eq]=${slug}&locale=${lang}&populate=*`, {
            mode: 'cors',
            credentials: 'omit'
        });
...
function renderRiseItem(item) {
    ...
    // 6. Social Sharing (reuse same logic)
...
async function loadRelatedContent(item) {
    ...
    try {
        const prevResponse = await fetch(`${CONFIG.API_URL}/rises?filters[date][$lt]=${item.date}&locale=${lang}&sort[0]=date:desc&pagination[limit]=1&populate=*`, {
            mode: 'cors',
            credentials: 'omit'
        });
        if (prevResponse.ok) {
    ...
    try {
        const relatedResponse = await fetch(`${CONFIG.API_URL}/rises?filters[category][$eq]=${item.category}&filters[id][$ne]=${item.id}&locale=${lang}&pagination[limit]=3&populate=*`, {
            mode: 'cors',
            credentials: 'omit'
        });
...
function renderPreviousPost(item) {
    ...
    const html = `
        <div class="nav-previous">
            <a class="nav-image-link" href="./?rise=${item.slug}">
                <img src="${imageUrl}" alt="${item.title}">
            </a>
            <div class="nav-desc">
                <h4>Previous Post</h4>
                <a href="./?rise=${item.slug}" rel="prev">
                    <span class="meta-nav">&#8592;</span> ${item.title}
                </a>
            </div>
            <a class="nav-overlay-link" href="./?rise=${item.slug}" rel="prev">
                ${item.title}
            </a>
        </div>
    `;
                    ...
                    <div class="post-wrap">
                        <header class="entry-header">
                            ...
                            <h2 class="entry-title">
                                <a href="./?rise=${item.slug}">${item.title}</a>
                            </h2>
                            <a class="more-link" href="./?rise=${item.slug}">View Post</a>
                        </header>
                    </div>
                </div>
            </div>
...
function renderRichText(content) {
    // Reuse the same logic as news-detail-loader.js because content structure is likely same
