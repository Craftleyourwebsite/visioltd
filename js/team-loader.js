document.addEventListener('DOMContentLoaded', () => {
    // First, hide all team slots immediately to prevent flash of empty content
    hideAllTeamSlots();
    fetchTeamMembers();
});

function hideAllTeamSlots() {
    for (let i = 1; i <= 4; i++) {
...
async function fetchTeamMembers() {
    try {
        ...
        if (members && members.length > 0) {
            ...
            // If there are more members, update the extended section
...
function updateMainTeam(members) {
    // Only handling 4 slots for the main section
    const maxSlots = 4;
    ...
    for (let i = 0; i < maxSlots; i++) {
        ...
        if (slot) {
            if (i < members.length) {
                ...
                // Strapi v5 returns flat structure, so we use member directly
                const attr = member;

                // Show the slot and its parent container
                ...
                // No member for this slot, hide it AND its parent container to fully remove visible frame
                slot.style.display = 'none';
                // Also hide the parent container (e-con e-child)
                const parentContainer = slot.closest('.e-con.e-child');
                if (parentContainer) {
                    parentContainer.style.display = 'none';
                }
    ...
    socialLinks.forEach(link => {
        ...
        if (svg.classList.contains('e-fab-facebook')) {
            if (attr.facebook_link) {
                href = attr.facebook_link;
                shouldShow = true;
            }
        } else if (svg.classList.contains('e-fab-twitter')) { // Twitter or X
            if (attr.twitter_link) {
                href = attr.twitter_link;
                shouldShow = true;
            }
        } else if (svg.classList.contains('e-fab-instagram')) {
            if (attr.instagram_link) {
                href = attr.instagram_link;
                shouldShow = true;
            }
        } else if (svg.classList.contains('e-far-envelope')) { // Email
            if (attr.email) {
        ...
        if (shouldShow) {
            link.href = href;
            link.style.display = 'inline-flex'; // or whatever the original display was (usually inline-block or inline-flex)
        } else {
...
function updateExtendedTeam(members) {
    ...
    if (members.length === 0) {
        // Optionally hide the extended section button if no extra members
        // const btn = document.querySelector('.qodef-m-see-more');
        // if(btn) btn.style.display = 'none';
        return;
    }

    // Clear existing static items in extended section if any (except the template if we were using cloning, but we'll generate HTML)
                ...
                <div class="qodef-m-inner">
                  <div class="qodef-m-image">
                    <div class="qodef-m-media-image">
                      <img loading="lazy" decoding="async" src="${photoUrl}" class="attachment-full size-full" alt="${attr.name}" style="width: 100%; height: auto; object-fit: cover; aspect-ratio: 1080/1215;">
                    </div>
                  </div>
                  <div class="qodef-m-content">
                    ...
                    <div class="qodef-m-social-icons">
                      ...
                      ${generateSocialLinkHTML(attr.instagram_link, 'instagram')}
                      ${generateSocialLinkHTML(attr.email, 'envelope', true)}
                    </div>
                  </div>
                </div>
              </div>
...
function generateSocialLinkHTML(link, type, isEmail = false) {
    ...
    // Simplified SVGs or classes for the dynamic part. 
    // Since we are generating HTML, we should ideally use the same SVG paths as the main design to match exact style.
    // Converting the existing inline SVGs to a map would be best.
    ...
    // Fallback or selection logic if type strings mismatch
